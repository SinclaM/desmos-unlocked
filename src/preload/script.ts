...
const handler = (({ detail }: CustomEvent<CommandMap>) => {
    let oldDefine!: any;
    function newDefine(moduleName: string, dependencies: any, definition: any) {
        if (moduleName === "mathquill_src") {
            ...
            dependencies = () => extendMathQuill(detail);
        }
  ...
  Now we load it, but with '??' appended to prevent the web request rules from blocking it */
    ...
    document.removeEventListener("send-remaps", handler);
}) as EventListener;

document.addEventListener("send-remaps", handler);
